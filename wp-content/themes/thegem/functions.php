...
function thegem_scripts() {
	...
	wp_register_script('thegem-blog', get_template_directory_uri() . '/js/thegem-blog.js', array('jquery', 'thegem-mediaelement', 'thegem-scroll-monitor', 'thegem-gallery'), '', true);
...
function thegem_page_title() {
	...
	$output .= '<div class="page-title-title" style="'.esc_attr($css_title_margin).'">'.($rich_title ? $rich_title : '<'.($title_style == '2' ? 'h2' : 'h1').' style="'.esc_attr($css_style_title).'"'.($xlarge ? 'class="title-xlarge"' : '').'>'.thegem_title  ('', false) .'</'.($title_style == '2' ? 'h2' : 'h1').'>').'</div>';
	if($excerpt) {
...
?>
	<div class="socials inline-inside">
		<?php foreach($socials_icons as $name => $active) : ?>
			<?php if($active) : ?>
				<a class="socials-item" href="<?php echo esc_url(thegem_get_option($name . '_link')); ?>" target="_blank" title="<?php echo esc_attr($thegem_socials_icons[$name]); ?>"><i class="socials-item-icon <?php echo esc_attr($name); ?> <?php echo ($type ? 'social-item-'.$type : ''); ?>"></i></a>
			<?php endif; ?>
		<?php endforeach; ?>
