...
class TGM_PageSpeed_Lazy_Items {
    ...
    public function processStyleBackground($matches) {
        ...
        $boxAttributes = array();
        if (preg_match_all('%(class|id)="([^"]*)"%i', $result, $boxMatches) && is_array($boxMatches[1])) {
        ...
        if (!empty($boxAttributes['class'])) {
            $result = preg_replace('%class="%', 'class="' . $backgroundClasses . ' ', $result);
        } else {
    ...
    public function processImage($matches) {
        ...
        if (preg_match_all('%(src|srcset|style|class|id|data\-ww)="([^"]*)"%i', $result, $imageMatches) && is_array($imageMatches[1])) {
            foreach ($imageMatches[1] as $index => $attr) {
                $imageAttributes[ $attr ] = $imageMatches[2][ $index ];
            }
        }

        if (empty($imageAttributes['src']) || !empty($imageAttributes['data-ww']) || (!empty($imageAttributes['class']) && preg_match('%(rev-slidebg)%i', $imageAttributes['class']))) {
            return $matches[0];
        }
        ...
        $result = preg_replace('%(src|srcset)=%', 'data-tgpli-${1}=', $result);
    ...
    public function processIframe($matches) {
        ...
        if (preg_match_all('%(src|style|class|id)="([^"]*)"%i', $result, $iframeMatches) && is_array($iframeMatches[1])) {
        ...
        $result = preg_replace('%(src)=%', 'data-tgpli-${1}=', $result);
