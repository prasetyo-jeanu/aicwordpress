...
function thegem_news_grid($params) {
	$params = array_merge(
		array(
			...
			'button' => array(),
			'metro_max_row_height' => 380,
			'grid_post_types' => array('post', 'thegem_news'),
	...
	if ($params['display_titles'] == 'hover' || $params['hover'] == 'gradient' || $params['hover'] == 'circular') {
		...
		$params['item_post_title_color'] = '';
		$params['item_post_date_color'] = '';
		$params['item_post_categories_color'] = '';
	}

	if ($params['version'] != 'new' || $params['display_titles'] == 'hover' || $params['hover'] == 'gradient' || $params['hover'] == 'circular') {
		$params['item_post_author_color'] = '';
		$params['item_post_excerpt_color'] = '';
		$params['item_post_categories_background_color'] = '';
		$params['item_post_comments_icon_color'] = '';
		$params['item_post_likes_icon_color'] = '';
		$params['item_post_share_icon_color'] = '';
		$params['item_post_share_social_icon_color'] = '';
	}
	...
	$taxonomies = array('category', 'thegem_news_sets');
	...
	switch ($params['version']) {
		...
		case 'new':
			wp_enqueue_style('thegem-news-grid-version-new-hovers');
			break;
	}
	...
	$inlineColorsStyles = '';
	$captionSelector = '.portfolio.news-grid[data-portfolio-uid="' . $grid_uid . '"] .portfolio-item .wrap > .caption';

	if (!empty($params['item_background_color'])) {
		$inlineColorsStyles .= $captionSelector. ' { background-color: ' . esc_attr($params['item_background_color']) . '; }';
	}
	if (!empty($params['item_post_author_color'])) {
		$inlineColorsStyles .= $captionSelector . ' .author .author-name { color: ' . esc_attr($params['item_post_author_color']) . '; }';
	}
	if (!empty($params['item_post_date_color'])) {
		$inlineColorsStyles .= $captionSelector . ' .post-author-date-separator { color: ' . esc_attr($params['item_post_date_color']) . '; }';
		$inlineColorsStyles .= $captionSelector . ' .post-date { color: ' . esc_attr($params['item_post_date_color']) . '; }';
	}
	if (!empty($params['item_post_categories_background_color'])) {
		$inlineColorsStyles .= $captionSelector. ' .info { background-color: ' . esc_attr($params['item_post_categories_background_color']) . '; }';
	}
	if (!empty($params['item_post_categories_color'])) {
		$inlineColorsStyles .= $captionSelector . ' .info .sep { color: ' . esc_attr($params['item_post_categories_color']) . '; border-left-color: ' . esc_attr($params['item_post_categories_color']) . '; }';
		$inlineColorsStyles .= $captionSelector . ' .info a { color: ' . esc_attr($params['item_post_categories_color']) . '; }';
	}
	if (!empty($params['item_post_excerpt_color'])) {
		$inlineColorsStyles .= $captionSelector . ' .description { color: ' . esc_attr($params['item_post_excerpt_color']) . '; }';
		$inlineColorsStyles .= $captionSelector . ' .grid-post-meta .comments-link a { color: ' . esc_attr($params['item_post_excerpt_color']) . '; }';
		$inlineColorsStyles .= $captionSelector . ' .grid-post-meta .zilla-likes { color: ' . esc_attr($params['item_post_excerpt_color']) . '; }';
	}
	if (!empty($params['item_post_comments_icon_color'])) {
		$inlineColorsStyles .= $captionSelector . ' .grid-post-meta .comments-link a:before { color: ' . esc_attr($params['item_post_comments_icon_color']) . '; }';
	}
	if (!empty($params['item_post_likes_icon_color'])) {
		$inlineColorsStyles .= $captionSelector . ' .grid-post-meta .zilla-likes:before { color: ' . esc_attr($params['item_post_likes_icon_color']) . '; }';
	}
	if (!empty($params['item_post_share_icon_color'])) {
		$inlineColorsStyles .= $captionSelector . ' .grid-post-share .icon { color: ' . esc_attr($params['item_post_share_icon_color']) . '; }';
	}
	if (!empty($params['item_post_share_social_icon_color'])) {
		$inlineColorsStyles .= $captionSelector . ' .socials-sharing a.socials-item { color: ' . esc_attr($params['item_post_share_social_icon_color']) . '; }';
	}
	...
	<?php if(!$params['is_ajax']) : ?>
		...
		<?php
			...
			$news_grid_classes = array(
				...
				'background-style-' . $params['background_style'],
				'title-style-' . $params['title_style'],
				'hover-' . ($params['version'] == 'new' ? 'new-' : '') . esc_attr($params['hover']),
				'item-animation-' . $params['loading_animation'],
				'title-on-' . $params['display_titles'],
			...
			if ($params['display_titles'] == 'page' && $params['hover'] == 'gradient') {
				$news_grid_classes[] = 'hover-gradient-title';
			}

			if ($params['display_titles'] == 'page' && $params['hover'] == 'circular') {
				$news_grid_classes[] = 'hover-circular-title';
			}
