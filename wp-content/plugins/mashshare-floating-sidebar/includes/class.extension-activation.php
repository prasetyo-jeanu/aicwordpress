 ...
 *
 * @package     MASHFS\ActivationHandler
 * @since       1.0.0
...
class MASHSB_Extension_Activation {
    ...
    public function missing_mashsb_notice() {
        if( $this->has_mashsb ) {
            echo '<div class="error"><p>' . $this->plugin_name . __( ' requires Mashshare! Please activate it to continue!', 'mashfs' ) . '</p></div>';
        } else {
            echo '<div class="error"><p>' . $this->plugin_name . __( ' requires Mashshare! Please install it to continue!', 'mashfs' ) . '</p></div>';
        }
    }
}