...
class Yoast_GA_Frontend_Custom_Dimensions extends Yoast_GA_Custom_Dimensions {
	...
	protected function set_dimension_output( $dimension ) {
		return "'set', 'dimension" . $dimension['id'] . "', '" . $dimension['value'] . "'";
	...
	protected function focus_keyword( $dimension ) {
		...
		if ( $this->wp_seo_active() && is_singular() ) {
			...
			if ( empty( $focus_keyword ) ) {
				$focus_keyword = _x( 'focus_keyword_not_set', 'Default value shown in Google Analytics when no focus keyword has been set. Uses underscores to differentiate from normal focus keywords. ', 'ga-premium' );
			}
