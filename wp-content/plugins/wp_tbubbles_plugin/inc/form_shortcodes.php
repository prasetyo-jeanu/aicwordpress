<?php
	...
	function tmls_form_shortcode($atts, $content=null) {  
		...
		extract(shortcode_atts( array(  
			...
			'imagefailed_message' => 'Avatar image has failed to upload',
			'selectimageagain_message' => '<b>Note:</b> Please select avatar image again',
			'logofailed_message' => 'logo image has failed to upload',
            'selectlogoagain_message' => '<b>Note:</b> Please select logo image again',
			...
			'rating_label_text' => 'Rating Stars',
		...
		if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["tmls_form_submit"]) && $_POST["tmls_form_submit"]) {
			...
			if (isset($_FILES["tmls_form_image"]) && $_FILES["tmls_form_image"]["name"] !='' && $tmls_error == false) {
				...
				if (($tmls_imagefiletype['type'] != "image/gif" && 
                    ...
                    $_FILES["tmls_form_image"]["size"] > 2097152)
				{
            ...
            if (isset($_FILES["tmls_form_logo"]) && $_FILES["tmls_form_logo"]["name"] !='' && $tmls_error == false) {
				
				$tmls_logofiletype = wp_check_filetype($_FILES["tmls_form_logo"]["name"]);
					
				if (($tmls_logofiletype['type'] != "image/gif" && 
				    $tmls_logofiletype['type'] != "image/jpeg" &&
				    $tmls_logofiletype['type'] != "image/jpg" && 
				    $tmls_logofiletype['type'] != "image/png" && 
				    $tmls_logofiletype['type'] != "image/pjpeg") ||
                    $_FILES["tmls_form_logo"]["size"] > 2097152)
				{
				    $tmls_error_message .= '<span>'.$logofailed_message.'</span>';
                    $tmls_error = true;
            ...
            if (isset($_FILES["tmls_form_image"]) && $_FILES["tmls_form_image"]["name"] !='' && $tmls_error == false) {
				...
				if ( $tmls_movefile && !isset( $tmls_movefile['error'] ) ) {
            ...
            if (isset($_FILES["tmls_form_logo"]) && $_FILES["tmls_form_logo"]["name"] !='' && $tmls_error == false) {
				...
				$tmls_logo_fileIsUpload = false;
				$tmls_logo_uploadedfile = $_FILES['tmls_form_logo'];
				$tmls_logo_upload_overrides = array( 'test_form' => false );
				$tmls_logo_movefile = wp_handle_upload( $tmls_logo_uploadedfile, $tmls_logo_upload_overrides );
						
				if ( $tmls_logo_movefile && !isset( $tmls_logo_movefile['error'] ) ) {
				    $tmls_logo_fileIsUpload = true;
				}
				else {
				    $tmls_error_message .= '<span>'.$logofailed_message.'</span>';
				    $tmls_error = true;
            ...
            if (isset($_FILES["tmls_form_image"]) && $_FILES["tmls_form_image"]["name"] !='' && $tmls_error == true) {
                $tmls_error_message .= '<span>'.$selectimageagain_message.'</span>';
            }
            
            if (isset($_FILES["tmls_form_logo"]) && $_FILES["tmls_form_logo"]["name"] !='' && $tmls_error == true) {
                $tmls_error_message .= '<span>'.$selectlogoagain_message.'</span>';
            }
			...
			if($tmls_error == false) {
				...
				if(isset($tmls_fileIsUpload) && $tmls_fileIsUpload==true) 
				{
                ...
                if(isset($tmls_logo_fileIsUpload) && $tmls_logo_fileIsUpload==true) 
				{
					$tmls_logo_attachment = array(
						'guid' => $tmls_logo_movefile['url'], 
						'post_mime_type' => $tmls_logofiletype['type'],
						'post_title' => preg_replace('/\.[^.]+$/', '', basename($tmls_logo_uploadedfile['name'])),
						'post_content' => '',
						'post_status' => 'inherit'
					);
					
					$tmls_logo_attach_id = wp_insert_attachment( $tmls_logo_attachment, $tmls_logo_movefile['url'], $tmls_post_id );
					
					if ( ! function_exists( 'wp_generate_attachment_metadata' ) ) require_once( ABSPATH . 'wp-admin/includes/image.php' );
					
					$tmls_logo_upload_dir = wp_upload_dir();
					$tmls_logo_attach_data = wp_generate_attachment_metadata( $tmls_logo_attach_id, $tmls_logo_upload_dir['path'].'/'.$tmls_logo_uploadedfile['name'] );
					wp_update_attachment_metadata( $tmls_logo_attach_id, $tmls_logo_attach_data );
                    
					update_post_meta( $tmls_post_id, 'logo', $tmls_logo_movefile['url'] );
				}
				...
				if($notificationemail=='enabled') {
					...
					mail($tmls_emailto_user_info->user_email, $emailsubject, $emailmessage,'From: '.$tmls_name.' <'.$tmls_email.'> '."\r\n".'MIME-Version: 1.0'."\r\n".'Content-type: text/html; charset=iso-8859-1'."\r\n");
		...
		if($is_generate_shortcode_page_form == false) {
			if($form_width < 350 || $position == 'disabled' || $rating == 'disabled') {
				$html.='<form autocomplete="off" id="tmls_form" method="post" action="'.$_SERVER['REQUEST_URI'].'#tmls_formmessage" class="tmls_form tmls_notready tmls_form_onecolumn" style="max-width:'.$form_width.'px;" enctype="multipart/form-data" >';
			}
			else {
				$html.='<form autocomplete="off" id="tmls_form" method="post" action="'.$_SERVER['REQUEST_URI'].'#tmls_formmessage" class="tmls_form tmls_notready" style="max-width:'.$form_width.'px;" enctype="multipart/form-data" >';
				$tmls_input_width = ($form_width-20)/2; 
			}
		}
		else if($is_generate_shortcode_page_form == true) {
			if($form_width < 350 || $position == 'disabled' || $rating == 'disabled') {
				$html.='<div class="tmls_form tmls_notready tmls_form_onecolumn" style="max-width:'.$form_width.'px;" >';
			}
			else {
				$html.='<div class="tmls_form tmls_notready" style="max-width:'.$form_width.'px;" >';
				$tmls_input_width = ($form_width-20)/2; 
			}
		}
			...
			$html.='	<div>
						...
						switch ($tmls_rating) {
							case "":
								...
								<option value="one_star">1</option>
								<option value="two_stars">2</option>
								<option value="three_stars">3</option>
								<option value="four_stars">4</option>
								<option value="five_stars">5</option>';
								break;
							case "one_star":
								$html.='
								<option value="">-</option>
								<option value="one_star" selected >1</option>
								<option value="two_stars">2</option>
								<option value="three_stars">3</option>
								<option value="four_stars">4</option>
								<option value="five_stars">5</option>';
							...
							case "two_stars":
								...
								<option value="">-</option>
								<option value="one_star">1</option>
								<option value="two_stars" selected >2</option>
								<option value="three_stars">3</option>
								<option value="four_stars">4</option>
								<option value="five_stars">5</option>';
								break;
							case "three_stars":
								...
								<option value="one_star">1</option>
								<option value="two_stars">2</option>
								<option value="three_stars" selected >3</option>
								<option value="four_stars">4</option>
								<option value="five_stars">5</option>';
								break;
							case "four_stars":
								...
								<option value="two_stars">2</option>
								<option value="three_stars">3</option>
								<option value="four_stars" selected >4</option>
								<option value="five_stars">5</option>';
								break;
							default:
								$html.='
								...
								<option value="three_stars">3</option>
								<option value="four_stars">4</option>
								<option value="five_stars" selected >5</option>';
						}
                ...
                if($logo=='enabled') {
					...
					<div>
						<div>
							<label for="tmls_form_logo" style="'.$tmls_labels_style.'">'.$logo_label_text.'</label>
							<input type="file" name="tmls_form_logo" id="tmls_form_logo" style="width:'.($tmls_input_width*2+20).'px;'.$tmls_inputs_style.'" class="'.$inputs_borderradius.'" />
						</div>
					</div>';	
				}
		...
		if($is_generate_shortcode_page_form == true) {
            ...
            if($logo=='enabled') {
				$html.='<span>'.$logofailed_message.'</span>';
				$html.='<span>'.$selectlogoagain_message.'</span>';		
			}
