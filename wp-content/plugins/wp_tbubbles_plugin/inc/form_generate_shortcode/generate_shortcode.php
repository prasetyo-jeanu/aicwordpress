<?php
	...
	function tmls_form_sc_options_metabox( $post ) {
        ...
        // form width
        $form_width = metadata_exists('post', $post->ID, 'form_width') ? get_post_meta($post->ID, 'form_width', true) : 450;
			
        // position
        $position = metadata_exists('post', $post->ID, 'position') ? get_post_meta($post->ID, 'position', true) : 'enabled';
			
        // company
        $company = metadata_exists('post', $post->ID, 'company') ? get_post_meta($post->ID, 'company', true) : 'enabled';
			
        // rating
        $rating = metadata_exists('post', $post->ID, 'rating') ? get_post_meta($post->ID, 'rating', true) : 'enabled';
			
        // image
        $image = metadata_exists('post', $post->ID, 'image') ? get_post_meta($post->ID, 'image', true) : 'disabled';
            
        // logo
        $logo = metadata_exists('post', $post->ID, 'logo') ? get_post_meta($post->ID, 'logo', true) : 'disabled';
			
        // notification email
        $notificationemail = metadata_exists('post', $post->ID, 'notificationemail') ? get_post_meta($post->ID, 'notificationemail', true) : 'disabled';
			
        // email to
        $emailto = metadata_exists('post', $post->ID, 'emailto') ? get_post_meta($post->ID, 'emailto', true) : 1;
			
        // email subject
        $emailsubject = metadata_exists('post', $post->ID, 'emailsubject') ? get_post_meta($post->ID, 'emailsubject', true) : 'A new testimonial has been received';
			
        // email message
        $emailmessage = metadata_exists('post', $post->ID, 'emailmessage') ? get_post_meta($post->ID, 'emailmessage', true) : 'A new testimonial message is waiting for approval.';
			
        // success message
        $success_message = metadata_exists('post', $post->ID, 'success_message') ? get_post_meta($post->ID, 'success_message', true) : 'Thank you! Your testimonial has been successfully sent';
			
        // name required message
        $namerequired_message = metadata_exists('post', $post->ID, 'namerequired_message') ? get_post_meta($post->ID, 'namerequired_message', true) : 'Name is required';
			
        // email required message
        $emailrequired_message = metadata_exists('post', $post->ID, 'emailrequired_message') ? get_post_meta($post->ID, 'emailrequired_message', true) : 'Email is required';
			
        // testimonial required message
        $testimonialrequired_message = metadata_exists('post', $post->ID, 'testimonialrequired_message') ? get_post_meta($post->ID, 'testimonialrequired_message', true) : 'Testimonial is required';
			
        // invalid email message
        $invalidemail_message = metadata_exists('post', $post->ID, 'invalidemail_message') ? get_post_meta($post->ID, 'invalidemail_message', true) : 'Invalid email format';
			
        // invalid company website message
        $invalidcompanywebsite_message = metadata_exists('post', $post->ID, 'invalidcompanywebsite_message') ? get_post_meta($post->ID, 'invalidcompanywebsite_message', true) : 'Invalid company website URL';
			
        // image failed message
        $imagefailed_message = metadata_exists('post', $post->ID, 'imagefailed_message') ? get_post_meta($post->ID, 'imagefailed_message', true) : 'Avatar image has failed to upload';
			
        // select image again message
        $selectimageagain_message = metadata_exists('post', $post->ID, 'selectimageagain_message') ? get_post_meta($post->ID, 'selectimageagain_message', true) : '<b>Note:</b> Please select avatar image again';
            
        // logo failed message
        $logofailed_message = metadata_exists('post', $post->ID, 'logofailed_message') ? get_post_meta($post->ID, 'logofailed_message', true) : 'Logo image has failed to upload';
			
        // select logo again message
        $selectlogoagain_message = metadata_exists('post', $post->ID, 'selectlogoagain_message') ? get_post_meta($post->ID, 'selectlogoagain_message', true) : '<b>Note:</b> Please select logo image again';
			
        // invalid captcha answer message
        $invalidcaptchaanswer_message = metadata_exists('post', $post->ID, 'invalidcaptchaanswer_message') ? get_post_meta($post->ID, 'invalidcaptchaanswer_message', true) : 'Invalid captcha answer';
			
        // captcha answer required message
        $captchaanswerrequired_message = metadata_exists('post', $post->ID, 'captchaanswerrequired_message') ? get_post_meta($post->ID, 'captchaanswerrequired_message', true) : 'Captcha answer is required';
			
        // already sent message
        $alreadysent_message = metadata_exists('post', $post->ID, 'alreadysent_message') ? get_post_meta($post->ID, 'alreadysent_message', true) : 'Your testimonial already sent';
			
        // label font color
        $label_fontcolor = metadata_exists('post', $post->ID, 'label_fontcolor') ? get_post_meta($post->ID, 'label_fontcolor', true) : '#999999';
			
        // label font size
        $label_fontsize = metadata_exists('post', $post->ID, 'label_fontsize') ? get_post_meta($post->ID, 'label_fontsize', true) : '13px';
			
        // label font weight
        $label_fontweight = metadata_exists('post', $post->ID, 'label_fontweight') ? get_post_meta($post->ID, 'label_fontweight', true) : 'normal';
			
        // label font family
        $label_fontfamily = metadata_exists('post', $post->ID, 'label_fontfamily') ? get_post_meta($post->ID, 'label_fontfamily', true) : '';
			
        // validation message font color
        $validationmessage_fontcolor = metadata_exists('post', $post->ID, 'validationmessage_fontcolor') ? get_post_meta($post->ID, 'validationmessage_fontcolor', true) : '#e14d43';
			
        // validation message font size
        $validationmessage_fontsize = metadata_exists('post', $post->ID, 'validationmessage_fontsize') ? get_post_meta($post->ID, 'validationmessage_fontsize', true) : '13px';
			
        // validation message font weight
        $validationmessage_fontweight = metadata_exists('post', $post->ID, 'validationmessage_fontweight') ? get_post_meta($post->ID, 'validationmessage_fontweight', true) : 'normal';
			
        // validation message font family
        $validationmessage_fontfamily = metadata_exists('post', $post->ID, 'validationmessage_fontfamily') ? get_post_meta($post->ID, 'validationmessage_fontfamily', true) : '';
			
        // success message font color
        $successmessage_fontcolor = metadata_exists('post', $post->ID, 'successmessage_fontcolor') ? get_post_meta($post->ID, 'successmessage_fontcolor', true) : '#a3b745';
			
        // success message font size
        $successmessage_fontsize = metadata_exists('post', $post->ID, 'successmessage_fontsize') ? get_post_meta($post->ID, 'successmessage_fontsize', true) : '13px';
			
        // success message font weight
        $successmessage_fontweight = metadata_exists('post', $post->ID, 'successmessage_fontweight') ? get_post_meta($post->ID, 'successmessage_fontweight', true) : 'normal';
			
        // success message font family
        $successmessage_fontfamily = metadata_exists('post', $post->ID, 'successmessage_fontfamily') ? get_post_meta($post->ID, 'successmessage_fontfamily', true) : '';
			
        // inputs font color
        $inputs_fontcolor = metadata_exists('post', $post->ID, 'inputs_fontcolor') ? get_post_meta($post->ID, 'inputs_fontcolor', true) : '#999999';
			
        // inputs font size
        $inputs_fontsize = metadata_exists('post', $post->ID, 'inputs_fontsize') ? get_post_meta($post->ID, 'inputs_fontsize', true) : '12px';
			
        // inputs font weight
        $inputs_fontweight = metadata_exists('post', $post->ID, 'inputs_fontweight') ? get_post_meta($post->ID, 'inputs_fontweight', true) : 'normal';
			
        // inputs font family
        $inputs_fontfamily = metadata_exists('post', $post->ID, 'inputs_fontfamily') ? get_post_meta($post->ID, 'inputs_fontfamily', true) : '';
			
        // inputs border color
        $inputs_bordercolor = metadata_exists('post', $post->ID, 'inputs_bordercolor') ? get_post_meta($post->ID, 'inputs_bordercolor', true) : '#eeeeee';
			
        // inputs bg color
        $inputs_bgcolor = metadata_exists('post', $post->ID, 'inputs_bgcolor') ? get_post_meta($post->ID, 'inputs_bgcolor', true) : 'transparent';
			
        // inputs border radius
        $inputs_borderradius = metadata_exists('post', $post->ID, 'inputs_borderradius') ? get_post_meta($post->ID, 'inputs_borderradius', true) : 'small_radius';
			
        // name label text
        $name_label_text = metadata_exists('post', $post->ID, 'name_label_text') ? get_post_meta($post->ID, 'name_label_text', true) : 'Name';
			
        // position label text
        $position_label_text = metadata_exists('post', $post->ID, 'position_label_text') ? get_post_meta($post->ID, 'position_label_text', true) : 'Position';
			
        // company name label text
        $companyname_label_text = metadata_exists('post', $post->ID, 'companyname_label_text') != null ? get_post_meta($post->ID, 'companyname_label_text', true) : 'Company Name';
			
        // company website label text
        $companywebsite_label_text = metadata_exists('post', $post->ID, 'companywebsite_label_text') ? get_post_meta($post->ID, 'companywebsite_label_text', true) : 'Company Website';
			
        // email label text
        $email_label_text = metadata_exists('post', $post->ID, 'email_label_text') ? get_post_meta($post->ID, 'email_label_text', true) : 'Email';
			
        // rating label text
        $rating_label_text = metadata_exists('post', $post->ID, 'rating_label_text') ? get_post_meta($post->ID, 'rating_label_text', true) : 'Rating Stars';
			
        // testimonial label text
        $testimonial_label_text = metadata_exists('post', $post->ID, 'testimonial_label_text') ? get_post_meta($post->ID, 'testimonial_label_text', true) : 'Testimonial';
			
        // image label text
        $image_label_text = metadata_exists('post', $post->ID, 'image_label_text') ? get_post_meta($post->ID, 'image_label_text', true) : 'Avatar Image';
        
        // logo label text
        $logo_label_text = metadata_exists('post', $post->ID, 'logo_label_text') ? get_post_meta($post->ID, 'logo_label_text', true) : 'Logo Image';
			
        // captcha label text
        $captcha_label_text = metadata_exists('post', $post->ID, 'captcha_label_text') ? get_post_meta($post->ID, 'captcha_label_text', true) : 'Are you Human?';
			
        // button text
        $button_text = metadata_exists('post', $post->ID, 'button_text') ? get_post_meta($post->ID, 'button_text', true) : 'SEND TESTIMONIAL';
			
        // button font color
        $button_fontcolor = metadata_exists('post', $post->ID, 'button_fontcolor') ? get_post_meta($post->ID, 'button_fontcolor', true) : '#999999';
			
        // button font size
        $button_fontsize = metadata_exists('post', $post->ID, 'button_fontsize') ? get_post_meta($post->ID, 'button_fontsize', true) : '12px';
			
        // button font weight
        $button_fontweight = metadata_exists('post', $post->ID, 'button_fontweight') ? get_post_meta($post->ID, 'button_fontweight', true) : 'bold';
			
        // button font family
        $button_fontfamily = metadata_exists('post', $post->ID, 'button_fontfamily') ? get_post_meta($post->ID, 'button_fontfamily', true) : '';
			
        // button border color
        $button_bordercolor = metadata_exists('post', $post->ID, 'button_bordercolor') ? get_post_meta($post->ID, 'button_bordercolor', true) : '#eeeeee';
			
        // button bg color
        $button_bgcolor = metadata_exists('post', $post->ID, 'button_bgcolor') ? get_post_meta($post->ID, 'button_bgcolor', true) : 'transparent';
			
        // button border radius
        $button_borderradius = metadata_exists('post', $post->ID, 'button_borderradius') ? get_post_meta($post->ID, 'button_borderradius', true) : 'small_radius';
			
        // button hover fontcolor
        $button_hover_fontcolor = metadata_exists('post', $post->ID, 'button_hover_fontcolor') ? get_post_meta($post->ID, 'button_hover_fontcolor', true) : '#999999';
			
        // button hover border color
        $button_hover_bordercolor = metadata_exists('post', $post->ID, 'button_hover_bordercolor') ? get_post_meta($post->ID, 'button_hover_bordercolor', true) : '#eeeeee';
			
        // button hover bg color
        $button_hover_bgcolor = metadata_exists('post', $post->ID, 'button_hover_bgcolor') ? get_post_meta($post->ID, 'button_hover_bgcolor', true) : '#f5f5f5';
		...
		<div id="tmls_gene_short_leftSidebar">
			...
			<div class="tmls_rowsContainer tmls_rowsContainerOpend" >
                ...
                <div class="row">
					<label for="tmls_logo">Logo</label>
					<select id="tmls_logo" name="tmls_logo">
						<option value="enabled" <?php if($logo == 'enabled'){ echo 'selected'; } ?> >Enabled</option>
						<option value="disabled" <?php if($logo == 'disabled'){ echo 'selected'; } ?> >Disabled</option>
					</select>
				</div>
			...
			<div class="tmls_rowsContainer" >
				...
				<div class="row tmls_image_option">
					<label for="tmls_image_label_text">Avatar Image Label Text</label>
					<input type="text" id="tmls_image_label_text" name="tmls_image_label_text" value="<?php echo $image_label_text; ?>" />
				</div>
                
                <div class="row tmls_logo_option">
					<label for="tmls_logo_label_text">Logo Image Label Text</label>
					<input type="text" id="tmls_logo_label_text" name="tmls_logo_label_text" value="<?php echo $logo_label_text; ?>" />
				</div>
			...
			<div class="tmls_rowsContainer" >
				...
				<div class="row tmls_image_option">
					<label for="tmls_imagefailed_message">Avatar Image Failed To Upload Message</label>
					<input type="text" id="tmls_imagefailed_message" name="tmls_imagefailed_message" value="<?php echo $imagefailed_message; ?>" />
				</div>
				
				
				<div class="row tmls_image_option">
					<label for="tmls_selectimageagain_message">Select Avatar Image Again Message</label>
                ...
                <div class="row tmls_logo_option">
					<label for="tmls_logofailed_message">Logo Image Failed To Upload Message</label>
					<input type="text" id="tmls_logofailed_message" name="tmls_logofailed_message" value="<?php echo $logofailed_message; ?>" />
				</div>
				
				
				<div class="row tmls_logo_option">
					<label for="tmls_selectlogoagain_message">Select Logo Image Again Message</label>
					<input type="text" id="tmls_selectlogoagain_message" name="tmls_selectlogoagain_message" value="<?php echo $selectlogoagain_message; ?>" />
				</div>
	...
	function tmls_form_sc_save_meta_box($post_id) 
	{
        ...
        /*----------------------------------------------------------------------
			logo
		----------------------------------------------------------------------*/
		if(isset($_POST['tmls_logo'])) {
			update_post_meta($post_id, 'logo', $_POST['tmls_logo']);
		}
        ...
        /*----------------------------------------------------------------------
			logo failed to upload message
		----------------------------------------------------------------------*/
		if(isset($_POST['tmls_logofailed_message'])) {
			update_post_meta($post_id, 'logofailed_message', $_POST['tmls_logofailed_message']);
		}
		
		/*----------------------------------------------------------------------
			select logo again message
		----------------------------------------------------------------------*/
		if(isset($_POST['tmls_selectlogoagain_message'])) {
			update_post_meta($post_id, 'selectlogoagain_message', $_POST['tmls_selectlogoagain_message']);
		}
        ...
        /*----------------------------------------------------------------------
			logo label text
		----------------------------------------------------------------------*/
		if(isset($_POST['tmls_logo_label_text'])) {
			update_post_meta($post_id, 'logo_label_text', $_POST['tmls_logo_label_text']);
		}
